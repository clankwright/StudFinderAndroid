...
- [medium] Address F-Droid reviewer linsui's change requests on MR !39185 (notes 3389791957 + inline suggestion 3389792598 at `metadata/org.bitanon.studfinder.yml:28`): (a) replace `commit: v15` with `commit: 8bb5dd2934725584c30d002cea38314cc7d54c92` (full SHA of `v15^{}` on origin, verified via `git ls-remote origin refs/tags/v15`); (b) add `subdir: app` under the Builds block (Android module lives in `app/`); (c) delete the `UpdateCheckData: app/build.gradle|versionCode\s+([0-9]+)|.|v$1` line (line 28 of MR-head YAML — reviewer's inline `\`\`\`suggestion:-0+0\`\`\`` is an empty-replacement delete; `UpdateCheckMode: Tags` doesn't need UpdateCheckData). Push the fix as a new commit to fork branch `add-org.bitanon.studfinder` via GitLab API (same path as SPEC 5.3); MR !39185 auto-picks it up. Extend `docs/test_fdroid_mr_yaml_in_source_branch` (or add to `test_fdroid_yaml.py`) with three assertions on the MR-head YAML: (i) `Builds[0].commit` matches `^[0-9a-f]{40}$` AND is not `v15`, (ii) `Builds[0].subdir == "app"`, (iii) no top-level `UpdateCheckData:` key. Run `fdroid lint metadata/org.bitanon.studfinder.yml` against the modified YAML if `fdroid` is on PATH (skip-cleanly if not). Reason: SPEC 5.4 reviewer feedback received 2026-05-27T12:56Z; gates F-Droid acceptance.
